...
This doesn't yield anything further, but it doesn't hurt to cast the wider net --- it makes the query more robust across time periods when amenity classifications may have been different. I'll stick with this broader set of amenity types.

I also need to standardize the chain names. The raw `name` field from OpenStreetMap can vary (e.g., "GBK" vs "Gourmet Burger Kitchen", or "Byron Hamburgers" vs "Byron"). A `case_when` mapping handles this.
...
gifski(frame_files, gif_file = "img/byron_animation.gif",
       width = 1600, height = 1400, delay = 1/3)
